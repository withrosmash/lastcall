...
(function () {
  ...
  const tiles = []; let loaded = 0, total = 0;
  ...
  for (let j = 0; j < 3; j++) for (let i = 0; i < 3; i++) {
    ...
    img.src = `https://services.arcgisonline.com/ArcGIS/rest/services/Canvas/World_Dark_Gray_Base/MapServer/tile/${Z}/${Y0 + j}/${X0 + i}`;
    tiles.push({ img, i, j });
  }
  function mapBg(c) {
    const MAP_H = H - 520;
    ...
    tiles.forEach(({ img, i, j }) => { if (img.complete && img.naturalWidth) c.drawImage(img, i * TS - 228, j * TS + oy, TS, TS); });
    c.fillStyle = 'rgba(0,0,0,.04)'; c.fillRect(0, 0, W, MAP_H);
    ...
    t.addColorStop(0, 'rgba(0,0,0,.72)'); t.addColorStop(1, 'rgba(0,0,0,0)');
    c.fillStyle = t; c.fillRect(0, 0, W, 220);
    c.restore();
  }
  function bloom(c) {
    let g = c.createRadialGradient(W / 2, H, 0, W / 2, H, W * .9);
    g.addColorStop(0, 'rgba(33,118,79,.42)'); g.addColorStop(.5, 'rgba(10,36,25,.22)'); g.addColorStop(1, 'rgba(0,0,0,0)');
  ...
  function route(c, x, y, w, h, lw = 11, under) {
    ...
    if (under) { c.strokeStyle = 'rgba(0,0,0,.55)'; c.lineWidth = (lw + 10) / s; path(); c.stroke(); }
    c.strokeStyle = '#7EE0C0'; c.lineWidth = lw / s; path(); c.stroke();
    STOPS.forEach(([a, b]) => { c.beginPath(); c.arc(a, b, (lw * 1.6) / s, 0, 7); c.fillStyle = '#F06C9B'; c.fill(); if (under) { c.lineWidth = 4 / s; c.strokeStyle = '#000'; c.stroke(); } });
    c.restore();
  }
  ...
  function drawCard(cv, o) {
    ...
    if (!only) { c.fillStyle = '#000'; c.fillRect(0, 0, W, H); }
    const boxes = {};
    const title = clip(o.title);
    const a = o.avatar;
    if ((o.layout || 'route') === 'route') {
      if (want('map')) { if (only) { c.fillStyle = '#000'; c.fillRect(0, 0, W, H); } mapBg(c); bloom(c); }
      if (want('title') && title) text(c, title, M, M + 58, `700 64px ${SANS}`, '#FFFFFF', true);
      ...
      ROUTE_ROWS.forEach((row, r) => row.forEach(([l, v, pink], i) => {
        ...
        text(c, l, x, y - 62, `600 28px ${SANS}`, '#A3A3A3');
        text(c, v, x, y, `700 76px ${SANS}`, pink ? '#F06C9B' : '#FFFFFF');
      }));
      if (want('place')) text(c, DATE, M, H - M - 40, `400 30px ${SANS}`, '#8A8A8A');
      if (want('wordmark')) text(c, 'Last Call', M, H - M + 10, `700 40px ${SANS}`, '#7EE0C0');
